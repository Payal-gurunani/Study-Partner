...
export default function NoteDetail() {
      ...
      <main className="flex-1 sm:ml-64 px-4 py-10 sm:px-8 overflow-x-auto">
        ...
        <div className="mb-4">
          <button
            ...
            className="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-md text-sm font-medium transition cursor-pointer"
          >
            {summary ? "View Summary" : "Summary not available"}
          </button>
        </div>
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
    <div className="bg-gray-800 p-6 rounded-lg w-11/12 max-w-lg relative max-h-[90vh] overflow-y-auto">
      <button
        onClick={() => setShowSummaryModal(false)}
        className="sticky top-0 right-0 ml-auto block text-gray-400 hover:text-red-500 text-xl bg-gray-800"
      >
        ×
      </button>
      ...
      <div className="sm:hidden fixed bottom-0 left-0 w-full bg-neutral-800 border-t border-gray-700 px-4 py-3 flex justify-around z-50">
        <button
          onClick={() =>
            navigate(`/notes/${note._id}/quiz/generate`, { state: { from: location.pathname } })
          }
